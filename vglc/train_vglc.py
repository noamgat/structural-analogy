...
from vglc.vglc_utils import transform_input_vglc
import numpy as np
...
class VGLC_dataset(Data.Dataset):
    """Faces."""

    def __init__(self, json_path, levels_dir, opt):
        reader = VGLCGameData(json_path, levels_dir)
        levels_scales = transform_input_vglc(json_path, levels_dir, opt)
        self.size = len(reader)
        self.num_channels = len(reader.sorted_tile_types)
        self.data = []
        for level_scales in levels_scales:
            pytorch_scales = []
            for one_scale in level_scales:
                # H,W,D -> D,H,W
                one_scale = np.moveaxis(one_scale, [0, 1, 2], [1, 2, 0])
                pytorch_scales.append(torch.from_numpy(one_scale))
            self.data.append(pytorch_scales)
        self.scale_image_sizes = [(im.shape[1], im.shape[2]) for im in self.data[0]]
...
if __name__ == '__main__':
    ...
    # main arguments
    parser.add_argument('--input_a_json', help='game json of a', required=True)
    parser.add_argument('--input_a_dir', help='game json of a', required=True)
    parser.add_argument('--input_b_json', help='input image path', required=True)
    parser.add_argument('--input_b_dir', help='input image path', required=True)
    ...
    #Set the image size to be the maximal dimension of both datasets
    vglc_a = VGLCGameData(opt.input_a_json, opt.input_a_dir)
    vglc_b = VGLCGameData(opt.input_b_json, opt.input_b_dir)
    vglc_a_max_dim = max([max(level.shape) for level in vglc_a.get_levels(VGLCLevelRepresentationType.RAW)])
    vglc_b_max_dim = max([max(level.shape) for level in vglc_b.get_levels(VGLCLevelRepresentationType.RAW)])
    opt.img_size = max(vglc_a_max_dim, vglc_b_max_dim)
    opt.max_size = opt.img_size
    ...
    dataset_a = VGLC_dataset(opt.input_a_json, opt.input_a_dir, opt)
    data_loader_a = DataLoader(dataset_a, shuffle=True, batch_size=opt.bs)

    dataset_b = VGLC_dataset(opt.input_b_json, opt.input_b_dir, opt)
    data_loader_b = DataLoader(dataset_b, shuffle=True, batch_size=opt.bs)
    ...
    opt_a.img_size = opt_a.max_size = vglc_a_max_dim
    opt_b.img_size = opt_b.max_size = vglc_b_max_dim

    size_arr = []
    for ii in range(0, opt.stop_scale + 1, 1):
        scale = math.pow(opt.scale_factor, opt.stop_scale - ii)
        size_arr.append((math.ceil(scale * opt_a.img_size), math.ceil(scale * opt_b.img_size)))

    opt_a.nzx = size_arr[0][0]
    opt_a.nzy = size_arr[0][0]
    opt_b.nzx = size_arr[0][1]
    opt_b.nzy = size_arr[0][1]
    in_s_a = torch.full([opt_a.bs, opt_a.nc_z, opt_a.nzx, opt_a.nzy], 0, device=opt.device)
    in_s_b = torch.full([opt_b.bs, opt_b.nc_z, opt_b.nzx, opt_b.nzy], 0, device=opt.device)
    ...
    while scale_num < opt.stop_scale + 1:
        ...
        opt_a.nfc = opt_b.nfc = opt.nfc
        opt_a.min_nfc = opt_b.min_nfc = opt.min_nfc

        if scale_num > opt.switch_scale:
            D_a, G_a = init_models(opt_a)
            D_b, G_b = init_models(opt_b)
            print("No Res !!!")
        else:
            D_a, G_a = init_models_res(opt_a)
            D_b, G_b = init_models_res(opt_b)
            print("Res !!!")
        ...
        opt_a.nzx = size_arr[len(Gs_a)][0]
        opt_a.nzy = size_arr[len(Gs_a)][0]
        opt_b.nzx = size_arr[len(Gs_a)][1]
        opt_b.nzy = size_arr[len(Gs_a)][1]
        ...
        for epoch in range(n_iters):
            for data_a, data_b in product(data_loader_a, data_loader_b):

                real_a = data_a[len(Gs_a)].cuda()
                real_b = data_b[len(Gs_b)].cuda()

                # Create fake from noise
                noise_ = generate_noise2([opt_a.bs, opt_a.nc_z, opt_a.nzx, opt_a.nzy], device=opt.device)

                if Gs_a == []:
                    noise_a = noise_
                    prev_a = torch.full([opt_a.bs, opt_a.nc_z, opt_a.nzx, opt_a.nzy], 0, device=opt.device)
                else:
                    prev_a = draw_concat(Gs_a, list(data_a), NoiseAmp_a, in_s_a, 'rand', opt_a)
                    noise_a = opt_a.noise_amp_a * noise_ + prev_a

                noise_ = generate_noise2([opt_b.bs, opt_b.nc_z, opt_b.nzx, opt_b.nzy], device=opt.device)

                if Gs_b == []:
                    noise_b = noise_
                    prev_b = torch.full([opt_b.bs, opt_b.nc_z, opt_b.nzx, opt_b.nzy], 0, device=opt.device)
                else:
                    prev_b = draw_concat(Gs_b, list(data_b), NoiseAmp_b, in_s_a, 'rand', opt_b)
                    noise_b = opt_b.noise_amp_b * noise_ + prev_b
                ...
                if Gs_a == []:
                    z_prev_a = generate_noise2([opt_a.bs, opt_a.nc_z, opt_a.nzx, opt_a.nzy], device=opt.device)
                else:
                    z_prev_a = draw_concat(Gs_a, list(data_a), NoiseAmp_a, in_s_a, 'rec', opt_a)

                if epoch == 0 and i == 0:
                    if Gs_a == []:
                        opt_a.noise_amp_a = opt.noise_amp_a = 1
                    else:
                        criterion = nn.MSELoss()
                        RMSE = torch.sqrt(criterion(real_a, z_prev_a))
                        opt_a.noise_amp_a = opt.noise_amp_a = opt.noise_amp_init * RMSE / opt.bs

                if Gs_b == []:
                    z_prev_b = generate_noise2([opt_b.bs, opt_b.nc_z, opt_b.nzx, opt_b.nzy], device=opt.device)
                else:
                    z_prev_b = draw_concat(Gs_b, list(data_b), NoiseAmp_b, in_s_b, 'rec', opt_b)

                if epoch == 0 and i == 0:
                    if Gs_b == []:
                        opt_b.noise_amp_b = opt.noise_amp_b = 1
                    else:
                        criterion = nn.MSELoss()
                        RMSE = torch.sqrt(criterion(real_b, z_prev_b))
                        opt_b.noise_amp_b = opt.noise_amp_b = opt.noise_amp_init * RMSE / opt.bs
                ...
                other_noise_a = generate_noise2([opt_a.bs, opt_a.nc_z, opt_a.nzx, opt_a.nzy], device=opt.device)
                other_noise_b = generate_noise2([opt_b.bs, opt_b.nc_z, opt_b.nzx, opt_b.nzy], device=opt.device)
        ...
        Gs_a.append(G_a)
        NoiseAmp_a.append(opt_a.noise_amp_a)
        ...
        Gs_b.append(G_b)
        NoiseAmp_b.append(opt_b.noise_amp_b)
